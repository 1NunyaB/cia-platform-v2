-- Idempotent ensure: investigation graph tables + RLS (fixes missing public.evidence_clusters /
-- PostgREST "schema cache" errors when 003_investigation_graph was not applied).
-- Requires prior migrations: 001 (entities, …), 005 (is_case_member / can_write_case include creator).
...
drop policy if exists "entity_categories select" on public.entity_categories;
...
drop policy if exists "entity_categories write" on public.entity_categories;
...
drop policy if exists "timeline_event_evidence select" on public.timeline_event_evidence;
...
drop policy if exists "timeline_event_evidence write" on public.timeline_event_evidence;
...
drop policy if exists "evidence_links select" on public.evidence_links;
...
drop policy if exists "evidence_links write" on public.evidence_links;
