...
import { resolveAndEnsureSourceProgram, searchSourcePrograms } from "@/services/source-program-catalog";
...
export async function GET(request: Request) {
  ...
  try {
    const programs = await searchSourcePrograms(supabase, q, 40);
...
export async function POST(request: Request) {
  ...
  try {
    const label = await resolveAndEnsureSourceProgram(supabase, raw);
