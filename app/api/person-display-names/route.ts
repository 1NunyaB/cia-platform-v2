...
import {
  resolveAndEnsurePersonDisplayName,
  searchPersonDisplayNames,
} from "@/services/person-display-name-catalog";
...
export async function GET(request: Request) {
  ...
  try {
    const names = await searchPersonDisplayNames(supabase, q, 40);
...
export async function POST(request: Request) {
  ...
  try {
    const label = await resolveAndEnsurePersonDisplayName(supabase, raw);
