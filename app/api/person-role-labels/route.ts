...
import {
  resolveAndEnsurePersonRoleLabel,
  searchPersonRoleLabels,
} from "@/services/person-role-label-catalog";
...
export async function GET(request: Request) {
  ...
  try {
    const roles = await searchPersonRoleLabels(supabase, q, 40);
...
export async function POST(request: Request) {
  ...
  try {
    const label = await resolveAndEnsurePersonRoleLabel(supabase, raw);
