...
type ActionRow = { id: string; label: string; normalized: string };
...
export function LegalActionCombobox({
  id,
  value,
  onChange,
  className,
  ...
  const [options, setOptions] = useState<ActionRow[]>([]);
  ...
  const fetchOptions = useCallback(async (q: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/legal-action-labels?q=${encodeURIComponent(q)}`, {
        credentials: "same-origin",
      });
      const json = (await res.json()) as { actions?: ActionRow[]; error?: string };
      if (!res.ok) throw new Error(json.error ?? "Failed to load");
      setOptions(json.actions ?? []);
    } catch {
      setOptions([]);
    } finally {
      setLoading(false);
  ...
  function selectRow(row: ActionRow) {
  ...
  async function selectAddNew() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/legal-action-labels", {
        method: "POST",
        credentials: "same-origin",
    ...
    <div ref={rootRef} className="relative z-[120]">
      <div className="relative">
        <Input
          ...
          onBlur={() => {
            ...
            scheduleCanonicalize(() => {
              ...
              setQuery(label);
              onChange(label);
              void fetch("/api/legal-action-labels", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ label }),
              }).catch(() => {
                /* catalog optional; action still on incident */
              });
            });
          }}
          onKeyDown={onKeyDown}
        ...
        <button
          ...
          aria-label="Show action options"
          onMouseDown={(e) => {
            e.preventDefault();
            setOpen((o) => !o);
      ...
      {open ? (
        ...
        >
          ...
          {!loading && options.length === 0 && !addNewLabel && query.trim() ? (
            <div className="px-3 py-2 text-xs text-muted-foreground">No matches. Type an action and pick “Add …”.</div>
          ) : null}
        </div>
      ) : null}
    </div>
