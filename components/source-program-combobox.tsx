...
type ProgramRow = { id: string; label: string; normalized: string };
...
export function SourceProgramCombobox(
  props: {
    id: string;
    name?: string;
  ...
  const { id, name = "source_program", defaultValue, "aria-describedby": ariaDescribedBy } = props;
  const listId = useId();
  ...
  const [options, setOptions] = useState<ProgramRow[]>([]);
  ...
  const fetchOptions = useCallback(async (q: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/source-programs?q=${encodeURIComponent(q)}`, { credentials: "same-origin" });
      const json = (await res.json()) as { programs?: ProgramRow[]; error?: string };
      if (!res.ok) throw new Error(json.error ?? "Failed to load");
      setOptions(json.programs ?? []);
    } catch {
      setOptions([]);
    } finally {
      setLoading(false);
  ...
  function selectRow(row: ProgramRow) {
  ...
  async function selectAddNew() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/source-programs", {
        method: "POST",
    ...
    <div ref={rootRef} className="relative z-[100]">
      <div className="relative">
        <Input
          ...
          autoComplete="off"
          placeholder="Search or type a program, show, or title"
          value={query}
          onChange={(e) => {
          ...
          onBlur={() => {
            ...
            scheduleCanonicalize(() => {
              ...
              setQuery(label);
              void fetch("/api/source-programs", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
        ...
        <button
          ...
          aria-label="Show program options"
          onMouseDown={(e) => {
            e.preventDefault();
            setOpen((o) => !o);
