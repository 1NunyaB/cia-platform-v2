...
type HostRow = { id: string; label: string; normalized: string };
...
export function SourceSiteCombobox(
  props: {
    id: string;
    name?: string;
  ...
  const { id, name = "source_url", defaultValue, "aria-describedby": ariaDescribedBy } = props;
  const listId = useId();
  ...
  const [options, setOptions] = useState<HostRow[]>([]);
  ...
  const fetchOptions = useCallback(async (q: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/source-site-hosts?q=${encodeURIComponent(q)}`, { credentials: "same-origin" });
      const json = (await res.json()) as { hosts?: HostRow[]; error?: string };
      if (!res.ok) throw new Error(json.error ?? "Failed to load");
      setOptions(json.hosts ?? []);
    } catch {
      setOptions([]);
    } finally {
      setLoading(false);
  ...
  const resolvedPreview = query.trim() ? resolveSourceSiteHostForStorage(query) : null;
  const hasExactOption =
    resolvedPreview &&
    resolvedPreview.normalized &&
    options.some(
      (o) =>
  ...
  function selectRow(row: HostRow) {
  ...
  async function selectAddNew() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/source-site-hosts", {
        method: "POST",
        ...
        <Input
          ...
          onBlur={() => {
            ...
            scheduleCanonicalize(() => {
              ...
              if (!trimmed) return;
              const { label } = resolveSourceSiteHostForStorage(trimmed);
              setQuery(label);
              void fetch("/api/source-site-hosts", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
      ...
      {open ? (
        ...
        >
          ...
          {!loading && options.length === 0 && !addNewLabel && query.trim() ? (
            <div className="px-3 py-2 text-xs text-muted-foreground">No matches. Type a host and pick “Add …”.</div>
          ) : null}
        </div>
      ) : null}
    </div>
