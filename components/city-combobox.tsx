...
type CityRow = { id: string; label: string; normalized: string };
...
export function CityCombobox({
  ...
  const [options, setOptions] = useState<CityRow[]>([]);
    ...
    async (q: string) => {
      ...
      try {
        const res = await fetch(
          `/api/city-options?state=${encodeURIComponent(sc)}&q=${encodeURIComponent(q)}`,
          { credentials: "same-origin" },
        );
        const json = (await res.json()) as { cities?: CityRow[]; error?: string };
  ...
  function selectCityRow(row: CityRow) {
  ...
  async function selectAddNew() {
    ...
    try {
      const res = await fetch("/api/city-options", {
        ...
        body: JSON.stringify({ state: sc, label: query.trim() }),
      });
        ...
        <Input
          ...
          onBlur={() => {
            ...
            scheduleCanonicalize(() => {
              ...
              setQuery(label);
              onChange(label);
              void fetch("/api/city-options", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ state: sc, label }),
              }).catch(() => {
                /* catalog optional; value still saved on incident */
              });
            });
          }}
          onKeyDown={onKeyDown}
