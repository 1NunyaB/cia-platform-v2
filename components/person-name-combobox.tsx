...
type NameRow = { id: string; label: string; normalized: string };
...
export function PersonNameCombobox({
  ...
  const [options, setOptions] = useState<NameRow[]>([]);
  ...
  const fetchOptions = useCallback(async (q: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/person-display-names?q=${encodeURIComponent(q)}`, {
        credentials: "same-origin",
      });
      const json = (await res.json()) as { names?: NameRow[]; error?: string };
      if (!res.ok) throw new Error(json.error ?? "Failed to load");
      setOptions(json.names ?? []);
    } catch {
      setOptions([]);
    } finally {
      setLoading(false);
  ...
  function selectRow(row: NameRow) {
  ...
  async function selectAddNew() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/person-display-names", {
        method: "POST",
        credentials: "same-origin",
        ...
        <Input
          ...
          onBlur={() => {
            ...
            scheduleCanonicalize(() => {
              ...
              setQuery(label);
              onChange(label);
              void fetch("/api/person-display-names", {
                method: "POST",
                credentials: "same-origin",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ label }),
              }).catch(() => {
                /* catalog optional; name still on incident */
              });
            });
          }}
          onKeyDown={onKeyDown}
